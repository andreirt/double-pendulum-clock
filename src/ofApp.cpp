...
void ofApp::setup(){
    ...
    this->titleLabel = this->gui->addLabel("title", this->currentStrings["doublependulumclock"], OFX_UI_FONT_LARGE);
...
void ofApp::reset()
{
    ...
    if (previousImage.loadImage("double_pendulum_clock.png")) {
    ...
    if (this->videoGrabber->isFrameNew()) {
        ...
        for (int i = 0; i < this->speed; i++) {
            ...
            for (int j = 0; j < this->height && (this->y + j) < (this->cameraHeight - 1); j++) {
                ...
                this->paintPixel( this->x1, this->y + j, pixels, alpha);
                this->paintPixel( this->x2, this->y + j, pixels, alpha);
            }
            ...
            if (this->step == 0) {
                ...
                if (this->x1 < 0) {
                    this->x1 = 1;
                    this->x2 = this->cameraWidth/2 + 1;
                    this->step = 1;
                }
            } else if (this->step == 1) {
                ++this->x1;
                ++this->x2;
                if (this->x1 == this->cameraWidth/4) {
                    // changes y and height, but keeps speed
                    this->pickParameters();
                }
                else if (this->x1 >= this->cameraWidth/2) {
                    this->x1 = this->cameraWidth/2 - 1;
                    this->x2 = this->cameraWidth - 1;
                    this->step = 2;
                }
            } else {
                --this->x1;
        ...
        if (ofGetElapsedTimef() - this->lastTimeImageWasSaved > this->intervalToSaveImage * 60) {
            ...
            this->screenImage.saveImage("double_pendulum_clock.png");
...
void ofApp::guiEvent(ofxUIEventArgs &e)
{
    if (e.getName() == this->ptStrings["save"]) {
        ...
        if(!e.getButton()->getValue()) {
            ...
            this->titleLabel->setLabel( this->currentStrings["doublependulumclock"] );
...
{
    char filename[64];
    sprintf( filename, "double_pendulum_clock_%04d_%02d_%02d_%02d_%02d_%02d.png", ofGetYear(), ofGetMonth(), ofGetDay(), ofGetHours(), ofGetMinutes(), ofGetSeconds() );
