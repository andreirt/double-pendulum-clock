...
void ofApp::reset(){
    ...
    if (previousImage.load("double_pendulum_clock.png")) {
    ...
    if (this->videoGrabber->isFrameNew()) {
        ...
        if (this->saveImageToggle)
        {
            if (ofGetElapsedTimef() - this->lastTimeImageWasSaved > this->intervalToSaveImage * 60) {
                ...
                this->screenImage.save("double_pendulum_clock.png");
...
void ofApp::draw(){
    ...
    if(configurationPanelShow == true){
        ...
        ImGui::Begin(this->currentStrings["doublependulumclock"].c_str());
        //ImGui::Text(this->currentStrings["doublependulumclock"].c_str());
